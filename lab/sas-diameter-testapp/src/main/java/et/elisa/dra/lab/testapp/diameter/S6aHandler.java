...
final class S6aHandler implements ServerListener {
    ...
    private S6aAnswer build(S6aRequest request) throws Exception {
        ...
        if (request instanceof PurgeUERequest pur) {
            return onPur(pur);
        }
    ...
    private PurgeUEAnswer onPur(PurgeUERequest pur) throws Exception {
        String username = Answers.usernameOf(pur);
        Answers.received(hss, "PUR", pur, "user=" + username);

        SubscriberState state = hss.find(username).orElse(null);
        long resultCode = state == null ? Answers.ER_USER_UNKNOWN : Answers.SUCCESS;
        PurgeUEAnswer pua = messages.createPurgeUEAnswer(pur,
                pur.getHopByHopIdentifier(), pur.getEndToEndIdentifier(), resultCode);
        sent("PUA", pur, Long.toString(resultCode), state == null ? "user unknown" : "purged");
        return pua;
    }
    ...
    private S6aAnswer unableToDeliver(S6aRequest request) throws Exception {
        ...
        if (request instanceof PurgeUERequest pur) {
            return messages.createPurgeUEAnswer(pur,
                    pur.getHopByHopIdentifier(), pur.getEndToEndIdentifier(),
                    Answers.UNABLE_TO_DELIVER);
        }
