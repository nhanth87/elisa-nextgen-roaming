...
public sealed interface Matcher {
    ...
    record HasApp(long appId) implements Matcher {

        @Override
        public boolean evaluate(RoutingContext ctx) {
            return ctx.appOrOp() == appId;
        }
    }
    ...
    record ProtocolIs(SignalingMessage.Protocol target) implements Matcher {

        @Override
        public boolean evaluate(RoutingContext ctx) {
            return ctx.protocol() == target;
        }
    }
    ...
    final class PathNames {
        ...
        public static boolean isKnownKey(String canonical) {
            return Set.of("IMSI", "MSISDN", "VISITED_PLMN", "FRAMED_IP", "APN",
                    "DEST_HOST", "DEST_REALM", "ORIG_HOST", "ORIG_REALM", "SESSION_ID",
                    "GT", "OPC", "DPC", "SSN").contains(canonical)
